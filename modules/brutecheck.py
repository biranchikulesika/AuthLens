...
GUESS_SPEED_PRESETS = {
    "1": ("Basic CPU", 1_000_000),
    "2": ("High-end GPU", 500_000_000),
    "3": ("GPU Cluster / Cloud Rig", 10_000_000_000),
}
...
def classify_risk(worst_case_seconds: float) -> Tuple[str, str]:
    """Return risk level and short explanation."""
    ...
    if worst_case_seconds < 2_592_000:
        return "MODERATE", "Cracked within a month"
    if worst_case_seconds < 31_557_600:
...
def print_simulation_report(result: Dict, charset_label: str, speed_label: str) -> None:
    """Pretty CLI report using rich."""
    ...
    table.add_row(
        "Cracking Speed",
        f"{speed_label} ({format_large_number(result['guesses_per_second'])} guesses/sec)",
    )
    table.add_row("", "")
    ...
    content = Group(
        table,
        "\n[bold yellow]Estimated Crack Time:[/bold yellow]",
        time_table,
...
def save_simulation_report(
    ...
    if output_file is None:
        ...
        output_file = os.path.join(output_dir, f"brute_{timestamp}.txt")
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        f.write(f"Total Search Space    : {format_large_number(result['search_space'])}\n\n")

        f.write("Estimated Crack Times:\n")
        f.write(f"   Best Case      : {seconds_to_readable(result['best_case_seconds'])}\n")
        f.write(f"   Average Case   : {seconds_to_readable(result['average_case_seconds'])}\n")
        f.write(f"   Worst Case     : {seconds_to_readable(result['worst_case_seconds'])}\n\n")
...
def run_brutecheck() -> None:
    """User-friendly CLI entry point."""
    console.print()
    console.print(
        Panel(
            "[bold red]BruteCheck - Brute Force Simulator[/bold red]",
            expand=False,
            border_style="red",
            padding=(0, 2),
        )
